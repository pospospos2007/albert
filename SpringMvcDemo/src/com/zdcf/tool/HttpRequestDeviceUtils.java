...
public class HttpRequestDeviceUtils {
	    ...
	    */  
		...
		public static boolean isMobile(HttpServletRequest request) {
			...
			Object isMobileObj=session.getAttribute("isMobile");
			
			//HttpSession session=request.getSession(true);
			//Object isMobileObj=session.getAttribute("isMobile");
			//Boolean isMobile=null;
			if (null==isMobileObj){
				isMobileObj=Boolean.valueOf(isMobileDevice(request));
				//isMobileObj=Boolean.TRUE;
				session.setAttribute("isMobile", isMobileObj);
				System.out.println("用户是否使用手机登陆："+isMobileObj.toString());
			}
			
			return ((Boolean)isMobileObj).booleanValue();
		}
